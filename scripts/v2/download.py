#!/usr/bin/env python3
"""Download every raw file declared in scripts/v2/config.py:SOURCES.

Derives URLs from the manifest (scripts/v2/catalog.py output). Files land in
data/raw/<subfolder>/<filename>. Skips already-downloaded files unless --force.

Replaces v1's DOWNLOAD_MAP in scripts/download.py. Any source declared in
config.SOURCES gets pulled here; no parallel list to keep in sync.

Usage:
    python3 scripts/v2/download.py                       # all sources, current year + prior
    python3 scripts/v2/download.py --all                 # all years
    python3 scripts/v2/download.py --sources weekly_rosters injuries
    python3 scripts/v2/download.py --years 2024 2025
    python3 scripts/v2/download.py --force               # re-download existing
"""
...
# Put v1 scripts dir first (for `config.RAW_DATA_PATH`); import v2 config
# module explicitly by a different name.
sys.path.insert(0, str(ROOT / "scripts"))

from config import RAW_DATA_PATH  # noqa: E402 — v1 scripts/config.py

# Import v2 config (scripts/v2/config.py) as a separate module
...
def load_manifest() -> dict:
    if not MANIFEST_PATH.exists():
        raise SystemExit(f"Manifest missing: {MANIFEST_PATH}. Run scripts/v2/catalog.py first.")
...
def main():
    ...
    print(f"v2 downloader: {len(files)} files across {len(source_ids)} sources")
    if args.dry_run:
