...
Usage:
    ...
    python3 scripts/build_db_nflreadpy.py --pbp                        # Play-by-play DB
...
def _fetch_game_stats(years):
    return _polars_to_pandas(nflreadpy.load_player_stats(years, summary_level="week"))


def _fetch_season_stats(years):
    return _polars_to_pandas(nflreadpy.load_player_stats(years, summary_level="reg"))
...
def _fetch_players(_years=None):
    return _polars_to_pandas(nflreadpy.load_players())


def _fetch_player_ids(_years=None):
    return _polars_to_pandas(nflreadpy.load_ff_playerids())


def _fetch_draft_picks(_years=None):
    return _polars_to_pandas(nflreadpy.load_draft_picks())


def _fetch_combine(_years=None):
    return _polars_to_pandas(nflreadpy.load_combine())


def _fetch_snap_counts(years):
    return _polars_to_pandas(nflreadpy.load_snap_counts(years))


def _fetch_depth_charts(years):
    return _polars_to_pandas(nflreadpy.load_depth_charts(years))


def _fetch_depth_charts_2025(_years=None):
    return _polars_to_pandas(nflreadpy.load_depth_charts([2025]))
